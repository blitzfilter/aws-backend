...
- Each outbound capability owns one `ports/<capability>.rs` file; `ports/mod.rs` only assembles exports. The admin ListingSource search use case owns its safe summary and enforces administrator authorization before its bounded reader transaction. Public collection search and public slug detail have separate public-only views, reader factories, and short service-owned read transactions; they allow anonymous principals and never reuse admin detail/search ports or checks.
