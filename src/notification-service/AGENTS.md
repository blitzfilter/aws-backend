...
- Capture one provider result, original timestamp, and original token; retry only finalize, never provider again in the same attempt. Finalize false means `LeaseLost`. Adapter must recognize an exact committed replay after response loss without another write. Root `migrations/20260907000000_notification_completion_receipt.sql` supplies receipt columns for production and all PostgreSQL fixtures; no separate fixture rail.
