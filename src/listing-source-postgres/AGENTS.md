...
- Each reader implementation owns one `readers/<capability>.rs` file; `readers/mod.rs` holds shared adapter state, helpers, and narrow reader re-exports. The bounded ListingSource search reader joins only business ListingSource/Party data and never selects provider configuration or crawler-local state. Public collection and slug readers share a private allowlist mapper, query only source/operator presentation fields, and set their timeout with transaction-local PostgreSQL state. The initial business schema maintains adapter-only `name_search` columns with triggers; no aggregate, row, or existing admin read exposes them.
