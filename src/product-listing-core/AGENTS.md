...
- `product_listing::ProductListing` is canonical aggregate. Fields private. Rehydrate boundary public for adapter crates. Auction context is optional and is not Auction aggregate membership; an asserted context may have no leaf facts. `product_listing_auction` owns validated lot number, one-based catalogue position, and precision-bearing `bidding_opens`/`scheduled_closes` plus exact `reported_closed_at`; exact instants and same-timezone source dates reject bidding-open-after-scheduled-close, while mixed or incomparable precision remains un-ordered.
