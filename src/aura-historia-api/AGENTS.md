...
- `webhooks/` owns direct Shopify and WooCommerce product webhook transport. The WooCommerce path uses `/api/v1/webhooks/woocommerce/{listingSourceId}`. Controllers pass raw provider bytes plus topic/signature headers to the WooCommerce-owned intake mapper; it verifies the signature before parsing, retains semantic product JSON, maps source fields to generic raw values, and commits raw capture. A `204` means durable accepted raw input, not completed canonical ProductListing mutation.
