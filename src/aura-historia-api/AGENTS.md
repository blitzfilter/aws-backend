...
- Public ProductListing text search uses a search-only `CachedFxRateSnapshotReader` over the pooled one-statement FX reader. `PRODUCT_LISTING_SEARCH_FX_CACHE_ENABLED` is a strict boolean and defaults to `false` during this rollout. When enabled, exact cursor IDs use a 512-entry immutable FIFO payload cache; first-page latest selections reuse only a cutoff-compatible value for a fixed 30 seconds. Missing/error values are never cached and disabled mode directly calls PostgreSQL. Similar/detail/financial-write and administrative/invariant-critical flows retain fresh transaction-scoped repository wiring. `PRODUCT_LISTING_SEARCH_PARALLEL_ENRICHMENT_ENABLED` is a strict boolean, default `false`; it selects concurrent source/user-state/assessment reads for this public-search handler only. Search then uses native OpenSearch hybrid BM25 plus KNN when relevance-sorted query embedding succeeds; embedding failure and explicit non-score sorts use BM25. ProductListing search runtime needs `OPENSEARCH_ENDPOINT_URL`; outside `STAGE=ephemeral`, it also needs `OPENSEARCH_USERNAME` and `OPENSEARCH_PASSWORD`. Black-box ProductListing search, partner-write, and Shopify webhook tests seed a complete persisted FX snapshot after each PostgreSQL fixture reset so current-FX reads exercise their real invariants.
