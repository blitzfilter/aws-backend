...
- `auth/` owns bearer auth extraction, Cognito JWT verification via cached JWKS, Aura access-token auth, active-user status verification, and mapping to `OperationContext`.
...
- Cognito session revocation requires `AURA_HISTORIA_COGNITO_USER_POOL_ID` and AWS credentials permitted to call `cognito-idp:ListUsers` and `cognito-idp:AdminUserGlobalSignOut` for that pool. The User Cognito adapter resolves Aura `UserId` through the Cognito `sub` attribute before sign-out; it does not assume `UserId` is the Cognito username.
