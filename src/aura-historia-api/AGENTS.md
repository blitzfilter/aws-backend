...
- Public ProductListing text search uses a search-only `CachedFxRateSnapshotReader` over the pooled one-statement FX reader and a search-only `CachedListingSourceSummaryReader` over the existing batch source/referral reader. Both are wired only into public search; similar/detail/financial-write and administrative/invariant-critical flows retain fresh direct readers or transaction-scoped repository wiring. `PRODUCT_LISTING_SEARCH_FX_CACHE_ENABLED` is a strict boolean and defaults to `false` during this rollout. When enabled, exact cursor IDs use a 512-entry immutable FIFO payload cache; first-page latest selections reuse only a cutoff-compatible value for a fixed 30 seconds. Source caching is constructed disabled during this iteration; when enabled by its later rollout config, it holds complete source/referral values for a fixed 60 seconds with 4,096 FIFO entries, 8 MiB accounted payload, 16 KiB per-entry admission, and a coarse batch-fill gate. Misses remain one batch; missing/error values and stale-on-error fallbacks are never cached. The caches are per API process and restarts clear them. `PRODUCT_LISTING_SEARCH_PARALLEL_ENRICHMENT_ENABLED` is a strict boolean, default `false`; it selects concurrent source/user-state/assessment reads for this public-search handler only. User state and content assessments remain direct authoritative reads. Search then uses native OpenSearch hybrid BM25 plus KNN when relevance-sorted query embedding succeeds; embedding failure and explicit non-score sorts use BM25. ProductListing search runtime needs `OPENSEARCH_ENDPOINT_URL`; outside `STAGE=ephemeral`, it also needs `OPENSEARCH_USERNAME` and `OPENSEARCH_PASSWORD`. Black-box ProductListing search, partner-write, and Shopify webhook tests seed a complete persisted FX snapshot after each PostgreSQL fixture reset so current-FX reads exercise their real invariants.
