...
- `partnerships` is one Party-scoped aggregate table with exact `ACTIVE`/`DISSOLVED` lifecycle state.
...
- Admin partnership search uses one bounded joined reader query with `created DESC, partnership_id DESC` keyset pagination and returns active Partnerships only; filters use `EXISTS` so member and source-grant counts stay complete. The `partnerships_created_id_idx` index must match that order.
