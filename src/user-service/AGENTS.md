...
- Admin Cognito-session revocation checks the persisted administrator and loads the target user's stored Cognito subject inside a short transaction, commits it, then invokes the `UserSessionRevoker` port; it never derives provider identity from `UserId` or holds PostgreSQL open during the external call. The operation is retry-safe because Cognito global sign-out has no active-session precondition.
