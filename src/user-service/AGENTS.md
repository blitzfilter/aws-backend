...
- Admin Cognito-session revocation checks the persisted administrator and target user inside a short transaction, commits it, then invokes the `UserSessionRevoker` port; it never holds PostgreSQL open during the external call. The operation is retry-safe because Cognito global sign-out has no active-session precondition.
