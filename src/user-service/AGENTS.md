...
- Cognito identities are validated opaque `(issuer, subject)` values, separate from `UserId`. Registration runs under `Principal::System`, generates an independent UUIDv7 `UserId`, and transactionally binds one identity to one user. The resolver maps verified JWT issuer/subject pairs to users.
...
- Admin role-removal, user-deletion, and user-suspension commands use a transaction-bound `UserAdminMutationGuard` so PostgreSQL can protect the last active administrator. Admin actor reads return only persisted active (not suspended) admins. Suspension reasons are trimmed, capped at 1,000 bytes, reject common credential markers, and are logged only after a committed result. Admin reactivation uses the same transaction-scoped authorization and User repository, clears only durable suspension state, skips repeat writes, and logs committed outcomes.
- Admin Cognito-session revocation checks the persisted administrator and loads the target user's complete stored Cognito `(issuer, subject)` identity inside a short transaction, commits it, then invokes the `UserSessionRevoker` port. The adapter rejects an issuer that does not match its configured pool before provider lookup; it never derives provider identity from `UserId` or holds PostgreSQL open during the external call. The operation is retry-safe because Cognito global sign-out has no active-session precondition.
