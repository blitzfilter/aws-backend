...
- Admin Partnership dissolution authorizes the actor, loads and dissolves only the Partnership in one UoW, uses an optimistic repository write only when lifecycle changes, and commits changed or already-dissolved outcomes for structured audit logs.
