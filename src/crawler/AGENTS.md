...
- Product normalization uses `product-listing-normalization` pure APIs for candidate plausibility. Availability returns resolved, no-assertion, unsupported, or typed fail-closed error outcomes. `listing_source_urls.crawler_disposition` is `ACTIVE`, `DORMANT_SOLD`, or `DORMANT_REMOVED`; scrape and schema-seed selection accept only `ACTIVE`. Only resolved `SoldOut` becomes `DORMANT_SOLD`, after durable raw UPSERT capture. Verified removal becomes `DORMANT_REMOVED` only after durable raw DELETE capture. Unsupported, missing, ambiguous, invalid, and non-sold availability remain active; transient failures never change disposition. Both dormant dispositions are absorbing for local crawler writes: spider rediscovery keeps dormant URL state, and stale completion, classification, failure, retry, or reset writes are explicit no-ops rather than reactivation or metadata overwrite. Every automatic scraper-local URL write fences on the candidate's observed raw-input SHA-256 with null-safe equality; a newer active raw capture makes an older completion, classifier, or error write a no-op.
