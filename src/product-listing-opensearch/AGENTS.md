...
- OpenSearch reads are ordinary readers. No transaction or unit of work. Auction context stays listing-local: documents carry optional resolved `auctionId`, `lotLabel`, `lotPosition`, and exact-only `lotBiddingOpensAt`, `lotScheduledClosesAt`, and `lotReportedClosedAt`. The exact `auctionId` filter matches only resolved membership; omitted IDs cover both no context and unresolved context.
