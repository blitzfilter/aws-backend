...
- OpenSearch-backed public ProductListing search captures a valuation instant and resolves the required immutable FX snapshot through `FxRateSnapshotReader`: first pages read latest at or before the instant, while cursor pages read the pinned ID. API composition may wrap only this public-search dependency in `CachedFxRateSnapshotReader`; exact IDs remain exact, and latest reuse is bounded by a cutoff-compatible monotonic TTL. The PostgreSQL adapter uses one statement, not an explicit service transaction. Similar, detail, financial-write, administrative, and other invariant-critical FX flows retain fresh transaction-scoped repositories. Source, authenticated-user-state, and current-assessment reads use one pure assembly path and may run concurrently only when composition selects `ProductListingSearchReadExecutionPolicy::Concurrent`; default remains sequential. User state and assessments stay authoritative batch reads.
