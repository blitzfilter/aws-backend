...
- OpenSearch-backed public ProductListing search captures a valuation instant and resolves the required immutable FX snapshot through the ordinary `FxRateSnapshotReader`: first pages read latest at or before the instant, while cursor pages read the pinned ID. The PostgreSQL adapter uses one statement, not an explicit service transaction. The handler then compiles one `ProductListingSearchReadRequest`; OpenSearch and embedding stay outside that read. Similar, detail, financial-write, and other invariant-critical FX flows retain transaction-scoped repositories. Source, authenticated-user-state, and current-assessment reads use one pure assembly path and may run concurrently only when composition selects `ProductListingSearchReadExecutionPolicy::Concurrent`; default remains sequential. User state and assessments stay authoritative batch reads.
