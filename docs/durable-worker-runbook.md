...
| Serialized SQS job | At most 16 KiB; explicit `schema_version = 2` |

Envelope fields are `schema_version`, `scope`, `job_type`, `payload`, `idempotency_key`, and `ordering_key`. Payloads carry compact IDs/revisions, not raw CDC rows, source JSON, notification content, or credentials. Schema 2 uses canonical TypeIDs for Aura object fields and object-derived keys while CDC continues to receive PostgreSQL UUID text. It deliberately ignores unknown additive envelope/payload fields. Required fields, exact scope/type/operation, canonical TypeIDs, positive versions, and derived-key equality remain strict. Schema 1, unsupported types, and wrong scopes are poison, not successful no-ops. There is no schema-1 or bare-UUID compatibility decoder. This does not relax strict upstream ProductListing event validation.
...
1. **Pause Sequin delivery first**, retaining its unacknowledged backlog. Before the schema-2 rollout, stop schema-1 producers and prove every affected stage has no retained schema-1 source or DLQ messages; schema-1 messages are deliberately not readable by the new binary. Drain or reset disposable development/ephemeral queues. Complete the database, Cognito, Stripe, OpenSearch, and queue reset gates in the [object-ID reset policy](object-ids.md#wire-and-reset-policy). Any non-disposable stage needs an approved drain/archive/replay plan; if emptiness or recovery cannot be proved, stop the rollout. Inventory all old processes and their **in-memory queues and in-memory DLQs**. Drain them while still running, or capture recoverable compact job identities in an approved durable replay store **before stopping**. There is no universal legacy exporter. If drain/capture cannot be proved, stop the cutover and record the recovery gap. Previously lost jobs cannot be recovered magically by SQS.
