...
Shopify orders every captured product operation, including ID-only `products/delete`, only with EventBridge `X-Shopify-Triggered-At`; WooCommerce uses supplied `date_modified_gmt`. The guard stores exact epoch-second/nanosecond pairs, operation-aware evidence, and a timestamp-free DELETE barrier. It never compares resource `updated_at` with Shopify trigger time. A missing timestamp-free DELETE blocks later UPSERT capture as `PROVIDER_SOURCE_ORDER_AMBIGUOUS`; a missing-time UPSERT after a known DELETE is blocked too. The blocked delivery receives no receipt or raw revision, so redrive remains possible. Shopify returns it as an SQS partial failure; WooCommerce returns its retryable conflict response. Operators inspect metadata only, reconcile provider truth, then redrive while queue/request retention permits it. Equal time with distinct evidence is a recoverable conflict. A newer timestamped UPSERT can restore after a known ordered withdrawal. The upgrade keeps WooCommerce legacy microsecond guards as `LEGACY` (equal buckets do not restore), clears incomparable Shopify resource clocks, and marks retained final Shopify DELETE revisions as an unknown barrier. Immutable raw revisions, receipts, and normalization heads are not changed. Pause old capture writers before this migration; old writers must not run against V2 guards.
