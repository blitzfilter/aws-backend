...
Shopify orders every captured product operation, including ID-only `products/delete`, only with EventBridge `X-Shopify-Triggered-At`. It never compares a resource `updated_at` with this trigger clock. A missing trigger supplies no order; malformed trigger metadata is invalid and acknowledged without capture. After a timestamp-free delete, a later update cannot be proved newer from receipt identity alone, so operators must reconcile provider truth if delivery order matters. Equal trigger time with distinct effective evidence (`UPSERT` versus `DELETE` included) is a recoverable source-order conflict. WooCommerce uses `date_modified_gmt` only when supplied in a captured payload; an ID-only delete has the same explicit weaker guarantee. A rollout migration clears preexisting Shopify `updated_at` watermark metadata; immutable raw revisions and normalization heads are not changed.
