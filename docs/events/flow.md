...
- `product-listing-normalization` is the exception: startup runs one bounded global pending-stream page to repair missed wake-ups. Later reconciliation turns alternate one worker-local non-durable FIFO continuation and one global cursor page when a continuation exists. Only clean capped drains enter that FIFO. Blocked or transient stream errors are reported and remain pending for a later global traversal; they never enter the FIFO. Every global page is limited to available FIFO capacity. When a full FIFO continuation is popped, its vacated slot stays reserved for the next global page and its immediate capped hint is suppressed only until authoritative traversal reoffers it. That bounded page can adopt all of its continuations and advance the cursor, so later global pages remain reachable. A successful terminal global page clears it, retry exhaustion retains it, and direct CDC wake-ups never move or cancel cursor/FIFO state. Missed timer ticks skip rather than burst. Once a reconciliation turn is due, at most one ready CDC job runs before it; after a global page, one ready CDC job gets a turn before another due reconciliation turn.
...
| `product_listing_raw_revisions` | INSERT | `product-listing-normalization` only. The typed wake-up carries raw stream ID, raw revision ID, and revision number; it never carries source JSON. The service drains the stream head in order; a terminal outcome writes raw progress atomically with zero or one canonical ProductListing state/event. Candidate-data normalization failures terminally `REJECTED`; a `System` normalizer configuration failure returns retryable `NORMALIZATION_CONFIGURATION_FAILED` before transaction/completion, leaving the raw revision and head pending. Startup runs one bounded global page; later reconciliation turns alternate one non-durable worker-local FIFO continuation and one global cursor page when one is queued. Only clean capped drains enter FIFO. Blocked or transient stream errors are reported and remain pending for a later global traversal; they never enter FIFO. Every global page is limited to available FIFO capacity. When a full FIFO continuation is popped, its vacated slot stays reserved for the next global page and its immediate capped hint is suppressed only until authoritative traversal reoffers it. That page adopts all bounded continuations and advances the cursor, so later global pages remain reachable; a successful terminal global page clears it. Direct wake-ups never move or cancel cursor/FIFO state. Once a reconciliation turn is due, at most one ready direct wake-up runs before it; after a global page, one ready direct wake-up gets a turn before another due reconciliation turn. Graceful shutdown stops without draining queued wake-ups. Raw revisions never route directly to projections, notifications, translation, embedding, content assessment, or matching. |
