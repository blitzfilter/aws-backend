...
- Root migration `migrations/20260907000000_notification_completion_receipt.sql` adds paired `completed_lease_token`/`completed_at`. Finalization fences the active lease, then recognizes an exact committed token/result/time receipt on retry without rewriting it. Reclaim clears that receipt; existing terminal rows keep null receipt columns, never invented tokens. Deploy this additive migration before delivery binaries; preserve it on rollback.
