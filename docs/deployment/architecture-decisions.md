...
Daemon panic hook never prints/chains the default hook: atomically publish sticky failure and an absolute process fence before unwinding can block or leak payloads. Watchdog never waits on workload/timeline locks. Review failure signals before connection cleanup. Both signals and repeated failures cannot extend deadlines. This exception is confined to the crawler executable, not a general permission for unsafe business/adaptor code. Non-Unix daemon is unsupported. Success or failed fencing is proved by observed process termination, not readiness state; cross-host takeover still requires confirmed termination or separately implemented fencing.
