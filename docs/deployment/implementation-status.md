...
| 04 | Cron/capture foundations accepted; iteration incomplete | Local commit follows. Cron runtime/wiring/PG and crawler bounded-custody reviews accepted. Crawler server/spider cancellation and full handover remain unimplemented |
...
Remaining04: crawler `server` still lacks safe check-only/signal lifecycle; `run_scraper_once` logs outcome without propagating failure to server. Aborting a pass can lose the collector join; accepted in-memory evidence is not a durable queue. Stop/join producers, close all senders, join collector before pool shutdown still needs implementation. Spider/review task ownership and false checkpoint completion need repair. No extra crawler candidate/preflight may be started with the current ignored-argument CLI. Cross-host controller ownership transfer, real Sequin/SQS rehearsal and full workspace tests remain unpassed. Reverting these code slices needs coordinated probe/config changes, never database/queue reset.
