...
- **Breaking:** WooCommerce webhook intake now verifies the untouched signed request body, retains only semantic product JSON, maps WooCommerce topic/status/stock fields into generic raw values, and commits a changed raw ProductListing revision before returning `204 No Content`. A successful `204` means durable raw acceptance; canonical ProductListing mutation/event processing is asynchronous in the `product-listing-normalization` worker. Partner ProductListing batch endpoints remain synchronous/direct.
