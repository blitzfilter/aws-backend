...
- Admin Partnership dissolution is available at `DELETE /api/v1/admin/partnerships/{partnershipId}`. It requires the persisted `ADMIN` role and semantically deletes the relationship: it atomically marks the Partnership `DISSOLVED` and removes all current membership and ListingSource-grant rows, so authorization ceases immediately. The Partnership row remains as a historical approved-application reference; Parties, ListingSources, users, ProductListings, and PartnershipApplications remain. Active-only Partnership collection reads exclude dissolved rows, detail reads retain history, and a future approved application for the Party reactivates the Partnership. It returns idempotent `204 No Content` for changed or already-dissolved rows, maps invalid IDs to `INVALID_UUID`, missing rows to `PARTNERSHIP_NOT_FOUND`, stale writes to `409 CONFLICT`, and uses `Cache-Control: no-store`.
