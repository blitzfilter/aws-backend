...
Public listing discovery contains active listings only. Withdrawn listings are not found by public detail (by ID or title slug returns `404 PRODUCT_LISTING_NOT_FOUND`, never `410`) and are deleted from the OpenSearch projection; restore rebuilds the projection. Public discovery does not expose a lifecycle filter. OpenSearch retains only each raw source `url`. Search and KNN batch-hydrate current ListingSource referral configuration from PostgreSQL, then derive `view_url`: Partnerize when configured, otherwise Aura UTM parameters.
